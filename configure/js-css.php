...
function add_vite_assets()
{
  ...
  foreach ($js_files as $handle => $file) {
    $js_uri = VITE_SERVER . '/' . 'src/' . $file;
    if (VITE_BUILD) {
      $js_uri = DIST_URI . '/' . $manifest['src/' . $file]['file'];
    }

    wp_register_script($handle, $js_uri, [], null, true);
    $vars = array(
  ...
  foreach ($css_files as $handle => $file) {
    $css_uri = VITE_SERVER . '/' . 'src/main.css';
    if (VITE_BUILD) {
      $css_uri = DIST_URI . '/' . $manifest['src/main.js']['css'][0];
    }

    wp_enqueue_style($handle, $css_uri, [], null);
  }
}
...
{
  $sprite_path = get_template_directory_uri() . '/assets/sprite.svg';
